...
var source = (() => {
  ...
  var parseMangaDetails = ($2, mangaId) => {
    ...
    secondaryTitles.push(decodeHTMLEntity($2("img", "div.fixed-img").attr("alt")?.trim() ?? ""));
    ...
    const tagSections = [{ id: "0", title: "genres", tags: arrayTags }];
  ...
  var McReaderInterceptor = class extends import_types3.PaperbackInterceptor {
    async interceptRequest(request) {
      request.headers = {
        ...request.headers ?? {},
        ...{
  ...
  var McReaderSource = class {
    ...
    async initialise() {
      ...
      Application.registerDiscoverSection(
        ...
        Application.Selector(this, "getNewSectionItems")
      );
      Application.registerDiscoverSection(
        {
          id: "latest_updates",
    ...
    async getMostViewedSectionItems(section, metadata) {
    ...
    checkCloudflareStatus(status) {
      if (status == 503 || status == 403) {
        if (!this.cloudflareBypassDone) {
          throw new import_types3.CloudflareError({ url: MCR_DOMAIN, method: "GET" });
        }
      }
    }
  };
  ...
  // src/McReader/main.ts
  var McReader = new McReaderSource();
